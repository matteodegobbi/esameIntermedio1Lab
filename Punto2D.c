...
int trovaIsolatoMigliore(const Punto2D aP[], int nP) {
    ...
    liberaBSP(root);
    return indiceMax;
} /*trovaIsolatoMigliore*/
/*Trova l'indice del punto più isolato, scegliendo l'algoritmo piu' opportuno in base al valore di
   $nP,se $nP e' minore di SOGLIA_ALGORITMO chiama trovaIsolatoLineare altrimenti chiama
   trovaIsolatoMigliore, questa soglia e' stata determinata empiricamente per ottimizzare il tempo
   di esecuzione IP aP[] e' l'array di punti tra cui cercare l'isolato IP nP e' la grandezza
   dell'array $aP OR indiceMax e' l'indice in $aP del Punto2D piu' isolato*/
int trovaIsolato(const Punto2D aP[], int nP) {
    if (nP < SOGLIA_ALGORITMO) {
        return trovaIsolatoLineare(aP, nP);
    }
    return trovaIsolatoMigliore(aP, nP);
} /*trovaIsolato*/
/*----------------------------------------
FUNZIONI DI PRINT/SCAN:
-----------------------------------------*/
